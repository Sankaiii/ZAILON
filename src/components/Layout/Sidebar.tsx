  ...
  return (
    <aside className="w-14 flex flex-col items-center py-2 gap-0.5 bg-ink-400/60 border-r border-white/[0.05] flex-shrink-0">
      {NAV.map(({ id, icon: Icon, label }) => {
        ...
        return (
          <button key={id} onClick={() => setView(id as ViewType)} title={label}
            className={`w-9 h-9 flex items-center justify-center rounded-lg transition-all duration-150 relative ${
              active ? 'bg-gold/15 text-gold' : 'text-white/30 hover:text-white/70 hover:bg-white/[0.06]'
            }`}>
      ...
      <button onClick={() => setView('settings')} title={t('settings')}
        className={`w-9 h-9 flex items-center justify-center rounded-lg transition-all duration-150 relative ${
          currentView === 'settings' ? 'bg-gold/15 text-gold' : 'text-white/30 hover:text-white/70 hover:bg-white/[0.06]'
        }`}>
        {currentView === 'settings' && <span className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 bg-gold rounded-r-full" />}
